...
model2 = Mymodel(30,1).to(DEVICE) #모델을 선언

path = './_save/'

################################세이브 로드############################################
model2.load_state_dict(torch.load(path +'torch13_state_dict.pt')) #모델을 불러온다 # 모델불러올때 model12.
   
###################################################################################

################################하단꺼 붙인 지점############################################
y_predict = (model2(x_test) >=0.5).float() #0.5보다 크면 1, 작으면 0
print(y_predict[:10])
